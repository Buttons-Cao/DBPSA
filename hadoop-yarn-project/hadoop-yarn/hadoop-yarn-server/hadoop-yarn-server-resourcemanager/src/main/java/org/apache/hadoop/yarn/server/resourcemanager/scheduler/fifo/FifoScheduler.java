...
public class FifoScheduler extends
	 ...
	 */
	private void assignContainers(FiCaSchedulerNode node, long AppArrivalTime, long deadline, int numOfBeingPreempted, float preemptionPriority) {
		...
		if (request != null) {
			...
			assignedContainers =
				assignContainer(node, application, priority,
					assignableContainers, request, NodeType.NODE_LOCAL, rmContext.getArrivalTime(), rmContext.getDeadline(),
					rmContext.getnumOfBeingPreempted(), rmContext.getPreemptionPriority());
		}
	...
	private int assignRackLocalContainers(FiCaSchedulerNode node,
		...
		if (request != null) {
			...
			assignedContainers =
				assignContainer(node, application, priority,
					assignableContainers, request, NodeType.RACK_LOCAL, rmContext.getArrivalTime(), rmContext.getDeadline(),
					rmContext.getnumOfBeingPreempted(), rmContext.getPreemptionPriority());
		}
		return assignedContainers;
	}
	...
	private int assignOffSwitchContainers(FiCaSchedulerNode node,
		...
		if (request != null) {
			assignedContainers =
				assignContainer(node, application, priority,
					request.getNumContainers(), request, NodeType.OFF_SWITCH, rmContext.getArrivalTime(), rmContext.getDeadline(),
					rmContext.getnumOfBeingPreempted(), rmContext.getPreemptionPriority());
		}
		return assignedContainers;
	}
	...
	private int assignContainer(FiCaSchedulerNode node, FiCaSchedulerApp application,
	                            ...
	                            ResourceRequest request, NodeType type, long AppArrivalTime, long deadline,
			...
			for (int i = 0; i < assignedContainers; ++i) {
				...
				Container container =
					BuilderUtils.newContainer(containerId, nodeId, node.getRMNode()
						.getHttpAddress(), capability, priority, null, AppArrivalTime, deadline, numOfBeingPreempted,
						preemptionPriority);
	...
	private synchronized void nodeUpdate(RMNode rmNode) {
		...
		if (Resources.greaterThanOrEqual(resourceCalculator, clusterResource,
			...
			assignContainers(node, rmContext.getArrivalTime(), rmContext.getDeadline(), rmContext.getnumOfBeingPreempted(),
