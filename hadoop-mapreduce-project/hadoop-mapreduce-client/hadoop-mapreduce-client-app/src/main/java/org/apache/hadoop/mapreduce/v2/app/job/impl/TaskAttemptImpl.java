...
public abstract class TaskAttemptImpl implements
	...
	public TaskAttemptImpl(TaskId taskId, int i,
		...
		// It seems appContext is redundant?
	...
	private int getMemoryRequired(Configuration conf, TaskType taskType) {
		...
		if (taskType == TaskType.MAP) {
			...
			memory = conf.getInt(MRJobConfig.REDUCE_MEMORY_MB, MRJobConfig.DEFAULT_REDUCE_MEMORY_MB);
		}
		return memory;
	}
	...
	private int getCpuRequired(Configuration conf, TaskType taskType) {
		int vcores = 1;
		if (taskType == TaskType.MAP) {
			vcores = conf.getInt(MRJobConfig.MAP_CPU_VCORES, MRJobConfig.DEFAULT_MAP_CPU_VCORES);
		} else if (taskType == TaskType.REDUCE) {
			vcores = conf.getInt(MRJobConfig.REDUCE_CPU_VCORES,	MRJobConfig.DEFAULT_REDUCE_CPU_VCORES);
		}
		return vcores;
	}
	...
	private static ContainerLaunchContext createCommonContainerLaunchContext(
		...
		ContainerLaunchContext container =
			ContainerLaunchContext.newInstance(localResources, environment, null,	serviceData, taskCredentialsBuffer, applicationACLs);
