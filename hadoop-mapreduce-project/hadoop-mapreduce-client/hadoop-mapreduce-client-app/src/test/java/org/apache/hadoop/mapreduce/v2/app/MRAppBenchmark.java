...
public class MRAppBenchmark {
    ...
    static class ThrottledMRApp extends MRApp {
        ...
        ThrottledMRApp(int maps, int reduces, int maxConcurrentRunningTasks, long arrivalTime, long deadline) {
            super(maps, reduces, true, "ThrottledMRApp", true, arrivalTime, deadline);
    ...
    public void benchmark1() throws Exception {
        ...
        System.out.println("Running benchmark with maps:" + maps +
                " reduces:" + reduces);
        run(new MRApp(maps, reduces, true, this.getClass().getName(), true, Time.now(),
                                ...
                                for (ResourceRequest req : askList) {
                                    ...
                                    for (int i = 0; i < numContainers; i++) {
                                        ...
                                        containers.add(Container.newInstance(containerId,
                                                ...
                                                req.getCapability(), req.getPriority(), null, Time.now(),
                                                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE), new Random().nextInt(3), 0));
                                    }
                                }
    ...
    public void benchmark2() throws Exception {
        ...
        run(new ThrottledMRApp(maps, reduces, maxConcurrentRunningTasks, Time.now(), Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE)));
    }
