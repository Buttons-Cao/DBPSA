...
public class TestMRApp {
    ...
    public void testZeroMaps() throws Exception {
        MRApp app = new MRApp(0, 1, true, this.getClass().getName(), true, Time.now(),
                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
        Job job = app.submit(new Configuration());
        app.waitForState(job, JobState.SUCCEEDED);
        app.verifyCompleted();
    ...
    public void testZeroMapReduces() throws Exception {
        MRApp app = new MRApp(0, 0, true, this.getClass().getName(), true, Time.now(),
                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
        Job job = app.submit(new Configuration());
        app.waitForState(job, JobState.SUCCEEDED);
    }
    ...
    public void testCommitPending() throws Exception {
        MRApp app = new MRApp(1, 0, false, this.getClass().getName(), true, Time.now(),
                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
        Job job = app.submit(new Configuration());
    ...
    public void testCompletedMapsForReduceSlowstart() throws Exception {
        MRApp app = new MRApp(2, 1, false, this.getClass().getName(), true, Time.now(),
                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
    ...
    public void testJobSuccess() throws Exception {
        MRApp app = new MRApp(2, 2, true, this.getClass().getName(), true, false, Time.now(),
                Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
    ...
    private final class MRAppWithHistory extends MRApp {
        public MRAppWithHistory(int maps, int reduces, boolean autoComplete,
            ...
            super(maps, reduces, autoComplete, testName, cleanOnStart, startCount, Time.now(),
                    Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
        }
