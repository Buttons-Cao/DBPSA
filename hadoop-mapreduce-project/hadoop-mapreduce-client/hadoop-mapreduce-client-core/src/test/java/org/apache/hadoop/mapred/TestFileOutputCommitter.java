...
public class TestFileOutputCommitter extends TestCase {
	...
	private void testRecoveryInternal(int commitVersion, int recoveryVersion)
		...
		conf.setInt(org.apache.hadoop.mapreduce.lib.output.
			...
			commitVersion);
		conf.setArrivalTime(Time.now());
		conf.setDeadline(Time.now()+ Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
		JobContext jContext = new JobContextImpl(conf, taskID.getJobID(),conf.getArrivalTime(), conf.getDeadline());
		...
		conf2.setInt(org.apache.hadoop.mapreduce.lib.output.
				FileOutputCommitter.FILEOUTPUTCOMMITTER_ALGORITHM_VERSION,
			recoveryVersion);
		conf2.setArrivalTime(Time.now());
		conf2.setDeadline(Time.now() + Long.parseLong(MRJobConfig.DEFAULT_JOB_DEADLINE));
		JobContext jContext2 = new JobContextImpl(conf2, taskID.getJobID(), conf2.getArrivalTime(), conf2.getDeadline());
