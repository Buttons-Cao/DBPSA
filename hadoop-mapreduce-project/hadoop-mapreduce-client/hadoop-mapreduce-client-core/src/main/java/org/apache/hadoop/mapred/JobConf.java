...
public class JobConf extends Configuration {
	...
	public long getArrivalTime(){
		long deafaltArrivalTime = Time.now();
		if (MRJobConfig.JOB_ARRIVALTIME != null){
			return getLong(MRJobConfig.JOB_ARRIVALTIME, deafaltArrivalTime);
		}else {
			return deafaltArrivalTime;
		}
	}

	public void setArrivalTime(long arrivalTime){
		setStrings(MRJobConfig.JOB_ARRIVALTIME, String.valueOf(arrivalTime));
	}

	public float getJobPreemptionPriority() {
		String prio = get(JobContext.PREEMPTION_PRIORITY);
		if (prio == null) {
			return 0;
		}
		return Float.valueOf(prio);
	}

	public void setPreemptionPriority(float preemptionPriority){
		setStrings(MRJobConfig.PREEMPTION_PRIORITY, String.valueOf(preemptionPriority));
	}
}
